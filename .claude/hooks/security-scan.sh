...
# Check for hardcoded passwords (JSON, single-quoted, and double-quoted code)
if grep -qiE '"password"[[:space:]]*:[[:space:]]*"[^"]+"' "$FILE_PATH" 2>/dev/null || \
   grep -qiE '(password|passwd|pwd)[[:space:]]*=[[:space:]]*'"'"'[^'"'"']+'"'"'' "$FILE_PATH" 2>/dev/null || \
   grep -qiE '(password|passwd|pwd)[[:space:]]*=[[:space:]]*"[^"]+"' "$FILE_PATH" 2>/dev/null; then
